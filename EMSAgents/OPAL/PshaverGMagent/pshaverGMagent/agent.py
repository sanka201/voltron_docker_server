...
class Pshavergmagent(Agent):
    ...
    def _handle_publish(self, peer, sender, bus, topic, headers, message):
        ...
        temptopic1='Microgrid/GLEAMM/RTACHREXTRA/all'
       # if topic == temptopic1:
         #   tag='controllable'
        #    self.total_consumption=message['true_power']
        #    print("total consumption............EV",self.total_consumption)
        temptopic2='control/EV/PeakShaver'
        #print("Peak_shaver",topic)
        if topic == 'devices/Microgrid/GLEAMM/RTACHREXTRA/all':
             self.total_consumption=message[0]['True_power']/1000
             print("total consumption............EV",self.total_consumption)
             self.simhour=message[0]['Simhour']
             self.simminute=message[0]["Min"]
             print('SIMHour.................................EV', self.simhour)
        if topic == temptopic2:
    ...
    def PeakShaver(self):
       ...
       # if self.Peakshaverthreashhold >=0 :
        ...
        if (self.Threshour==self.simhour):
            if self.Peakshaverthreashhold <= 4:
                      result=self.vip.rpc.call('platform.driver','set_point', 'Microgrid/GLEAMM/RTACHREXTRA' ,'Profile_option',0)
                      time.sleep(1)
                      result=self.vip.rpc.call('platform.driver','set_point', 'Microgrid/GLEAMM/RTACHREXTRA' ,'Optype_EV',1)
            elif self.Peakshaverthreashhold > 4 and self.Peakshaverthreashhold <= 8:
                      result=self.vip.rpc.call('platform.driver','set_point', 'Microgrid/GLEAMM/RTACHREXTRA' ,'Profile_option',2)
                      time.sleep(1)
                      result=self.vip.rpc.call('platform.driver','set_point', 'Microgrid/GLEAMM/RTACHREXTRA' ,'Optype_EV',1)
            elif self.Peakshaverthreashhold > 8 and self.Peakshaverthreashhold <= 12:
                      result=self.vip.rpc.call('platform.driver','set_point', 'Microgrid/GLEAMM/RTACHREXTRA' ,'Profile_option',1)
                      time.sleep(1)
                      result=self.vip.rpc.call('platform.driver','set_point', 'Microgrid/GLEAMM/RTACHREXTRA' ,'Optype_EV',1)
