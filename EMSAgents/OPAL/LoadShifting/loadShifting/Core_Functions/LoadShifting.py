...
class LoadShiftingGM(LoadShifting):
        ...
        def __calc_total_rated_consumption(self):
            for i in self.__schedule:
                   ...
                   # print("UT",self.__schedule[i]['UT'])
        def __objective_function(self,x0,CURRENT,THREAHHOLD):
           # val =np.sum(np.square(self.__RATED_LOAD_CONSUMPTION['CT1']*x0+CURRENT-THREAHHOLD))
            val = np.square( -np.sum(self.__RATED_LOAD_CONSUMPTION['CT1']*x0)+self.__differableLoadAmount)
            return val
        ...
        def __shift_loads_scipy(self):
            ...
            cons2 = {'type': 'ineq', 'fun': lambda x:  -x*consumption-current+threshold+1000}
            bnds = []
            for k in range(0,window):
                bnds.append((0,1))
            x0=np.zeros(window)
            sol = minimize(self.__objective_function,x0,args=(current,threshold),method='SLSQP',bounds=bnds) #,constraints=[cons2])
            print('Solution',consumption*sol.x,threshold,sol)
            for i in self.__WINDOW:
                k=0
