...
class Loadshifting(Agent):
    ...
    def __init__(self, setting1=1, setting2="some/random/topic", **kwargs):
        ...
        self.profilepath='/home/pi/volttron/LoadShifting/Prof_C_csv.csv'
...
#            THRESHOLD={0:25,1:22,2:22,3:22,4:23,5:24,6:25,7:26,8:27,9:28,10:22,11:22,12:25,13:23,14:20,15:20,16:25,17:17,18:18,19:22,20:22,21:22,22:23,23:24}
            self.WINDOW=[(int(k),int(v)) for k,v in message[0]['Window']]
  #          self.shiftload(THRESHOLD,1)
            self.shiftload({int(k):int(v) for k,v in message[0]['Threshold'].items()},int(message[0]['Day']))
#            Message=[{'Threashhold':message[0]['Threashhold']},{'unit':'kw'}]
 #           result=self.vip.pubsub.publish(peer='pubsub',topic='dataconcentrator/devices/control/'+self.instancename.split('_')[0]+self.instancename.split('_')[1]+'/PeakShaver',message=Message).get(timeout=60)                
      ...
      #          self.prevhour=message[0]['Hour']
                print("Receving time ******************************************************",message)
                if  message[0]['Hour']==0 and message[0]['Day']==0:
                   Message=1
                   print("Resetting ********************************************************************************************************** BEMS",self.instancename)
                   result=self.vip.pubsub.publish(peer='pubsub',topic='dataconcentrator/devices/control/'+self.instancename.split('_')[0]+self.instancename.split('_')[1]+'/PeakShaver',message=100000).get(timeout=60)
                   result=self.vip.rpc.call('platform.driver','set_point', 'Campus1/Benshee1/'+self.instancename,'CMDC_G4',Message).get(timeout=20)
                   result=self.vip.rpc.call('platform.driver','set_point', 'Campus1/Benshee1/'+self.instancename,'CMDC_G5',Message).get(timeout=20)
                   result=self.vip.rpc.call('platform.driver','set_point', 'Campus1/Benshee1/'+self.instancename,'CMDC_G6',Message).get(timeout=20)
                   result=self.vip.rpc.call('platform.driver','set_point', 'Campus1/Benshee1/'+self.instancename,'CMDC_G7',Message).get(timeout=20)
                   result=self.vip.rpc.call('platform.driver','set_point', 'Campus1/Benshee1/'+self.instancename,'CMDC_G8',Message).get(timeout=20)
                   result=self.vip.rpc.call('platform.driver','set_point', 'Campus1/Benshee1/'+self.instancename,'CMDC_G9',Message).get(timeout=20)
                   result=self.vip.rpc.call('platform.driver','set_point', 'Campus1/Benshee1/'+self.instancename,'CMDC_G10',Message).get(timeout=20)
                   result=self.vip.rpc.call('platform.driver','set_point', 'Campus1/Benshee1/'+self.instancename,'CMDC_G1',Message).get(timeout=20)
                   result=self.vip.rpc.call('platform.driver','set_point', 'Campus1/Benshee1/'+self.instancename,'CMDC_G2',Message).get(timeout=20)
                   result=self.vip.rpc.call('platform.driver','set_point', 'Campus1/Benshee1/'+self.instancename,'CMDC_G3',Message).get(timeout=20)
                  # result=self.vip.pubsub.publish(peer='pubsub',topic='dataconcentrator/devices/control/'+self.instancename.split('_')[0]+self.instancename.split('_')[1]+'/PeakShaver',message=100000).get(timeout=60)
    ...
    def setload(self,schedule,hour):
        ...
        for k in temp.keys():
            if k=='CT10':
                ...
                print('seting..................',tag,result,hour,self.differableLoadAmount,self.shiftedLoadAmount,schedule[k],self.Pn_kW)
            elif k=='UT':
                pass
            else:
 ...
 #               result=self.vip.rpc.call('platform.driver','set_point', 'Campus1/Benshee1/'+self.instancename,tag,temp[k]).get(timeout=60)                
        ...
        if hour in range(self.WINDOW[0][0],self.WINDOW[0][1]):
               Message=[{'Threashhold':self.Threashhold[hour]-int(schedule['CT10']),'Hour':hour},{'unit':'kw','unit':'hour'}]
        else:
               Message=[{'Threashhold':self.Threashhold[hour]-int(schedule['CT10']),'Hour':hour},{'unit':'kw','unit':'hour'}]
        result=self.vip.pubsub.publish(peer='pubsub',topic='dataconcentrator/devices/control/'+self.instancename.split('_')[0]+self.instancename.split('_')[1]+'/PeakShaver',message=Message).get(timeout=60)                
    ...
    def shiftload(self,THRESHOLD,Day,initiate='None'):
        Pn_kW=self.Pn_kW
        LOADS={'CT1':Pn_kW, 'CT2':Pn_kW, 'CT3':Pn_kW, 'CT4':Pn_kW, 'CT5':Pn_kW, 'CT6':Pn_kW, 'CT7':Pn_kW, 'CT8':Pn_kW, 'CT9':Pn_kW, 'CT10':Pn_kW,'UT':Pn_kW}
        PRIORITY_LIST={'CT1':1, 'CT2':2, 'CT3':3, 'CT4':4, 'CT5':5, 'CT6':6, 'CT7':7, 'CT8':8, 'CT9':9, 'CT10':0,'UT':1000}
        ...
        for k in THRESHOLD.keys():
           self.Threashhold[k] =THRESHOLD[k]-self.schedule[Day][k]['UT']
           if self.Threashhold[k]<0:
              self.Threashhold[k]=0
        #print('Original',self.instancename,self.schedule[Day],'controlable threashhold',self.Threashhold)
        print('Original','controlable threashhold',self.Threashhold)
        for i in range(self.WINDOW[0][0],self.WINDOW[0][1]):
              self.Threashhold[i]= self.Threashhold[i]+200
        schedule=r.ReadScheduleCSV(self.profilepath,LOADS,7)
#        print('Schedule***********************************************************************************************',schedule)
        self.schedule=schedule.read_rated_consumption()
        print('Original',self.instancename,self.schedule[Day])
        loadshifter=LS.LoadShiftingGM(self.schedule[Day],self.Threashhold,PRIORITY_LIST,LOADS,self.WINDOW)
