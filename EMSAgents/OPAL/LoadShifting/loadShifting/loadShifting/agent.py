...
#            Message=[{'Threashhold':message[0]['Threashhold']},{'unit':'kw'}]
 #           result=self.vip.pubsub.publish(peer='pubsub',topic='dataconcentrator/devices/control/'+self.instancename.split('_')[0]+self.instancename.split('_')[1]+'/PeakShaver',message=Message).get(timeout=60)                
    ...
    def setload(self,schedule,hour):
        ...
        for k in temp.keys():
            if k=='CT10':
                ...
                result=self.vip.rpc.call('platform.driver','set_point', 'Campus1/Benshee1/'+self.instancename,tag2,int(10000*schedule[k]/self.Pn_kW),schedule[k],self.Pn_kW).get(timeout=60)
        ...
        Message=[{'Threashhold':self.Threashhold[hour]-int(schedule['CT10'])},{'unit':'kw'}]
        result=self.vip.pubsub.publish(peer='pubsub',topic='dataconcentrator/devices/control/'+self.instancename.split('_')[0]+self.instancename.split('_')[1]+'/PeakShaver',message=Message).get(timeout=60)                
    ...
    def shiftload(self,THRESHOLD,Day,initiate='None'):
        ...
        loadshifter=LS.LoadShiftingGM(self.schedule[Day],THRESHOLD,PRIORITY_LIST,LOADS,self.WINDOW)
