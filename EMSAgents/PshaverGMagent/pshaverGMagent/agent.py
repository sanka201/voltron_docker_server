...
class Pshavergmagent(Agent):
    ...
    def __init__(self, setting1=1, setting2="some/random/topic", **kwargs):
        ...
        self.instancename='NIRE_WeMo_cc_1'
        ...
        self.core.periodic(90,self.PeakShaver)
    ...
    def _handle_publish(self, peer, sender, bus, topic, headers, message):
        ...
        """
        print("Peak_shaver",topic,str(self.instancename))
        temptopic1=str(self.instancename)+'/TotalWeMoConsumption'
        if topic == temptopic1:
            tag='controllable'
            self.total_consumption=int(message)
            print("total consumption............",self.total_consumption)
        temptopic2='control/plc/'+str(self.instancename)+'/PeakShaver'
        ...
        if topic == temptopic2:
            ...
            Message=[{'Threshold':self.Peakshaverthreashhold},{'unit':'w'}]
            self.vip.pubsub.publish(peer='pubsub',topic='devices/building540/NIRE_WeMo_cc_1/Cluster',message=Message)
            self.PeakShaver()
        if topic == str(self.instancename)+'/PeakShaver/Driectcontrol/all':
            print("***********************got it******************Pshaver Direct all ",message)
	   # for i in range(1,30):
#                result=agent.vip.rpc.call('platform.driver','set_point','building540/NIRE_WeMo_cc_1/w'+str(i),'priority',i)
            #   result=self.vip.rpc.call('platform.driver','set_point','building540/NIRE_WeMo_cc_1/w'+str(i),'status',int(message)).get(timeout=20)
    def PeakShaver(self):
        ...
        if shedding >0 and self.Peakshaverthreashhold>=0:

           topics='control/plc/'+str(self.instancename)+'/shedding'
        ...
        if shedding <0 and self.Peakshaverthreashhold>=0:
           ...
           topics='control/plc/'+str(self.instancename)+'/increment'
 ...
 #          result=self.vip.rpc.call('platform.driver','set_point', 'Campus1/Benshee1/BEMS_'+str(self.BEMStag),'CMDC_BKR',1).get(timeout=20)
#           result=self.vip.rpc.call('platform.driver','set_multiple_points', 'Campus1/Benshee1/BEMS_'+str(self.BEMStag),[('CMDC_G1',0),('CMDC_G2',0),('CMDC_G3',0),('CMDC_G4',0),('CMDC_G5',0),('CMDC_G6',0),('CMDC_G7',0),('CMDC_G8',0),('CMDC_G9',0),('CMDC_G10',0)]).get(timeout=20)  
